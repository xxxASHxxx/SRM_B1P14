package com.quantitymeasurement;

import com.quantitymeasurement.model.Quantity;
import com.quantitymeasurement.model.WeightUnit;
import org.junit.Test;
import static org.junit.Assert.*;

public class QuantityWeightTest {

    @Test
    public void given0KilogramAnd0Kilogram_ShouldReturnEqual() {
        Quantity<WeightUnit> kg1 = new Quantity<>(0.0, WeightUnit.KILOGRAM);
        Quantity<WeightUnit> kg2 = new Quantity<>(0.0, WeightUnit.KILOGRAM);
        assertEquals(kg1, kg2);
    }

    @Test
    public void given1KilogramAnd1000Grams_ShouldReturnEqual() {
        Quantity<WeightUnit> kg1 = new Quantity<>(1.0, WeightUnit.KILOGRAM);
        Quantity<WeightUnit> g1000 = new Quantity<>(1000.0, WeightUnit.GRAM);
        assertEquals(kg1, g1000);
    }

    @Test
    public void given1PoundAndGrams_ShouldReturnEqual() {
        Quantity<WeightUnit> lb1 = new Quantity<>(1.0, WeightUnit.POUND);
        Quantity<WeightUnit> g453_592 = new Quantity<>(453.592, WeightUnit.GRAM);
        assertEquals(lb1, g453_592);
    }

    @Test
    public void givenWeights_WhenAdded_ShouldReturnSum() {
        Quantity<WeightUnit> kg1 = new Quantity<>(1.0, WeightUnit.KILOGRAM);
        Quantity<WeightUnit> g1000 = new Quantity<>(1000.0, WeightUnit.GRAM);
        Quantity<WeightUnit> result = Quantity.add(kg1, g1000);
        assertEquals(new Quantity<>(2.0, WeightUnit.KILOGRAM), result);
    }

    @Test
    public void givenWeight_WhenConverted_ShouldReturnNewConvertedQuantity() {
        Quantity<WeightUnit> kg1 = new Quantity<>(1.0, WeightUnit.KILOGRAM);
        Quantity<WeightUnit> inGrams = kg1.convertTo(WeightUnit.GRAM);
        assertEquals(new Quantity<>(1000.0, WeightUnit.GRAM), inGrams);
    }
}
